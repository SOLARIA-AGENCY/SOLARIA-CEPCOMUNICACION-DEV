...
// --- COMPONENTE DE PÁGINA DE CURSO REUTILIZABLE ---
const CursoPageComponent: React.FC<{ curso: typeof cursoData[0] }> = ({ curso }) => {
  ...
  return (
    <div className="bg-gray-50 min-h-screen">
        ...
        <div className="container mx-auto px-4 py-12 md:py-16">
          <div className="max-w-4xl mx-auto">
            <div className="bg-white rounded-lg shadow-lg p-6 mb-12 flex flex-wrap items-center justify-around gap-6 -mt-32 relative z-10">
              <div className="flex items-center gap-3"><Calendar className="w-8 h-8 text-cep-primary" /><div><p className="text-sm text-gray-500">Inicio</p><p className="font-bold text-gray-900">{inicio}</p></div></div>
              <div className="flex items-center gap-3"><MapPin className="w-8 h-8 text-cep-primary" /><div><p className="text-sm text-gray-500">Sede</p><p className="font-bold text-gray-900">{sede}</p></div></div>
              <div className="flex items-center gap-3"><Award className="w-8 h-8 text-cep-primary" /><div><p className="text-sm text-gray-500">Certificación</p><p className="font-bold text-gray-900">Diploma CEP</p></div></div>
            </div>
            <div className="grid md:grid-cols-3 gap-12">
              <div className="md:col-span-2 prose prose-lg max-w-none">
                ...
                <p>{copy.textosPrincipales[0]}</p><p>{copy.textosPrincipales[1]}</p>
                <h3 className="text-2xl font-bold mt-10">¿Qué aprenderás?</h3>
                <ul className="space-y-2">{copy.titulos.map((titulo, i) => (<li key={i} className="flex items-start"><CheckCircle className="w-6 h-6 text-green-500 mr-3 mt-1 flex-shrink-0" /><span>{titulo}</span></li>))}</ul>
              ...
              <aside className="md:col-span-1">
                <div className="bg-white rounded-lg shadow-lg p-6 sticky top-24">
                  ...
                  <button onClick={() => setIsModalOpen(true)} className="w-full bg-cep-primary text-white font-bold py-3 px-4 rounded-lg hover:bg-cep-primary/90 transition-all duration-300 transform hover:scale-105">¡Inscríbete Ahora!</button>
      ...
      <CursoInscripcionModal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} curso={{ nombre: curso.nombre, sede: curso.sede, tag: curso.tag }} />
      <CepFooter />
    </div>
  );
};
